...
def extract_still_frame(video_path, interval_seconds, start_video, end_video, threshold):
    ...
    for frame_num in range(start_video, end_video, frame_interval):
        ...
        if all(compute_frame_difference(frame, clip.get_frame(t / fps)) <= threshold for t in
               range(frame_num, frame_num + frame_interval)):
            image = Image.fromarray(np.uint8(frame * 255))
            image.save(f'./pictures_youtube/still_frame{frame_num}.jpg')
            print(f"Изображение still_frame{frame_num}.jpg сохранено.")

    clip.close()


async def download_picture_from_video(video_path: str, interval_seconds: int, threshold: float | int):
    clip = VideoFileClip(video_path)
    fps = clip.fps
